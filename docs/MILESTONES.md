...
| [M37](M37-plan.md) | 07-28 | beat the 600 band: anti-stall race mode + band-weighted gate | PLANNED (post-mortem-driven; plan v1 + P0a experiments done in the planning session, Windows box). ENV: m28_winners.pt reconstructed from submission npz (measurement-faithful: luc 0.6875/arch 0.9700/wall 0.3950 reproduce m36 pins); rocket/grim clone ckpts + ship tarballs still Linux-only; hermes CLI absent. P0a: stall lists extracted from the 55030954 cache — `decks/hops_stall.csv` (2 variants Δ1 card), `decks/cynthia_garchomp.csv` (1 list, 2 teams), `decks/grim_live.csv` (dominant 5-ep list). **Strawman: hop bed VALID** (solver 0.470 n=200; diag 11/18 losses true deck-outs = live farm mode; 1 win by winning the deck-out race); **garchomp INVALID at every cheap rung** (solver 0.665/solver2 0.710/solver-dev 0.615 vs live 0-2); **grim_live INVALID** (0.790 vs live 0.375) → opponent PILOTING is the bottleneck; clone recipe required (data viable: cynthia 512 / hop 262 / grim 378 seat-rows; live opp subs pinned in plan). Band gate implemented (`scripts/band_decode.py`, frozen n=52 weights: 73% coverage); ship-config composite **0.6378 ± 0.030 = UPPER BOUND** (invalid cells inflate; live band WR 0.500). gacfv pre-cleared on hop (0.4775 v 0.4700, z+0.15). O12 racemode designed (trigger id sets pinned; M36 raceconserve margin gate generalized). Test suite 651/655 on Windows (4 portability artifacts, flagged). Awaiting Piotr: arm pick (racemode vs gacfv-fast-ship), clone build-vs-sync, grim deck-tech appetite. | — | 📝 planned |
