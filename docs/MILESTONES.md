...
| [M27](M27-plan.md) | TBD | generalize "set up before you attack": free-actions-first turn discipline | STUB (07-22, from the M26 root-cause read): supporter backstop (Boss's Orders PLAY cell 0.065, the anti-stall tool) → evolve backstop → temptation-state weighting (weight rows where teacher chose a FREE action with a turn-ending option on the menu) → turn-phase masking if needed; battery gains a real wall/stall deck (the 0.31 teacher-level hole has NO offline instrument); parked: low-deck Dudunsparce draw guard (deviates from teacher). Phase 0 = 54903635 live read. **NEW HARD RULE: pre-ship human QC — 3 bundle games saved to replays/ (ptcgvis) + Piotr's manual review before ANY submit** (also in CLAUDE.md). | — | 📝 stub |
