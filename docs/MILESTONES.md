...
High-level map of every milestone (M0–M43) for orientation; each section links to the
full diary. **Campaign bar:** since M39 the target is **implied ELO 1000** on the live
leaderboard; the M8-era 0.55-vs-`solver:lucario` bar is pre-fix era (see the epoch marker
above; re-pinned in M38 G2 — the corrected champion reads .839). Since M10 the **meta_v2
co-gate** on promotion, since M18 every gate baseline **2-seed pooled**, since M39 panels
over single beds (G-13). Current live state (M41b, 2026-08-05, a deliberate two-deck A/B,
not a succession): sub **55265099** = `checkpoints/m41b_wide_prod.pt` (width 143) +
`decks/alakazam_v2_h4.csv`, and sub **55265105** = `checkpoints/m41_ogerpon.pt` +
`decks/ogerpon.csv` (unchanged re-ship). Best settled live implied ELO to date: **817**
(M30 / M35). In flight: **M43** (`docs/M43-plan.md`) — self-play corpus + the campaign's
first honest PPO measurement.

(The mermaid map and per-milestone notes below still end at M21/M24; the one-line summary
table is the maintained index — M27 onward live there and in their `docs/MXX.md` diaries.)
...
| [M27](M27.md) | 07-22 | RE-AIM after M26: is the clone's defect turn-ending discipline? | three read-only probes over `m25_bc_alakazam_v3h` + its exact corpus (16,086 contested decisions), live field decomposition of sub 54903635, and a top-teacher swap test | **thesis FALSIFIED**: clone ends the turn 12.9% vs teacher 12.1% (+0.8pp), keeps a free action on 92.3% of teacher-free rows — the M26 "Boss's Orders → ATTACK ×16" gun was a tail on a **31-row** cell; the real defect is **supporters 3.8% vs 7.5% (0.51×) and stadiums 0.4% vs 6.6% (1 of 272)** on identical menus; live Darkness cell **0.18 (2W–9L) vs 0.54 elsewhere, Fisher p=0.029**; "clone the top-scoring teacher" KILLED (grim clone 0.148 vs dragapult, less than half `m27_both`'s 0.3738) | 🔬 NO-SHIP research milestone (Piotr, 07-22) | teacher fidelity declared **exhausted** (six milestones, no gate moved past noise); Darkness = the one structurally-evidenced lever (+3–4pp ceiling) but no valid instrument; `scripts/turn_discipline_report.py`, `scripts/trainer_play_report.py`; the pre-ship human QC rule |
| [M28](M28.md) | 07-22→23 | three axes off the M27 dead end: instrument, process, data | A Darkness sparring instrument · B MCTS · C PPO · E value function · F `--winners-only` data · G phase conditioning — each a controlled A/B on the clone base | **only the DATA change worked: winners-only +6.3pp mirror, resolved, out-of-loop flat**; MCTS −11.0pp, PPO −9.0pp, better value leaf −25.8pp, phase conditioning flat and harmful when composed (0.7256→0.6525); the Darkness gate is **unbuildable** — every pilot we own sits 0.29–0.378 on `grimmsnarl_dark` while our clone beats `rule:lucario` at 0.686 | 🔬 no ship ("not a ship candidate on this evidence alone") | **`m28_winners` — the base checkpoint every ship from M29 to M37 rides on**; "do not build the Darkness gate" recorded as a kill (an advisory gate proven to be a strawman launders our own weakness); every *process* change failed, the one *data* change worked |
| [M29](M29.md) | 07-23 | scale the winner-demonstration lever; ship | refresh grew the same-deck (`9294d9d8`) winner pool 203→786 → `m29_pooled_winners` (783 winner games / 57,209 rows, 8 teachers, m28_winners' exact recipe, corpus the only variable) | **dragapult 0.4450 n=400 z=+3.13 RESOLVED — campaign out-of-loop record** (pin 0.3375, prev best 0.3738); mirror flat 0.676; grim 0.734 advisory; kyogre 0.9625; pre-registered mirror bar NOT met — shipped on the resolved out-of-loop record + Piotr's overnight go | 🚀 shipped 54914673 | first **pre-ship human QC** ever executed (replays/m29_qc_pooled_*); M28 addenda resolved: winners-only +6.3pp confirmed, MCTS/PPO/value-fn/phase all resolved negative |
| [M30](M30.md) | 07-23 | deck-economy O-rules + a single-teacher data arm, on pre-registered split bars | O4 deckguard / O5 ash / O6 conserve behind card-fact semantics checks; arm D = 54773249-winners retrain; `deck_drain.py` forensics after a QC no-go | **O6 conserve: true deck-outs 50%→37.5%, rocket 0.5675 n=400 z=+2.11 resolved-better, mirror 0.675 n=800 non-inferior, QC round 2 3W–0L**; arm D killed offline on the M29 pattern (drag z=+3.71 / rocket z=+2.75 but mirror z=−2.01); b1ga/b1tempo killed on behavioral claims | 🚀 shipped 54929991 | split ship bars (rule arms = non-inferiority, weight arms = resolved-better) proved their worth; the rocket clone is the **first offline opponent to reproduce the live stall loss mode**; implied ELO 817 — best sub of the campaign at the time |
| [M31](M31.md) | 07-23 | bench economy as a deterministic rule, base weights frozen | O7 `poffinfloor` (bench ≤1 & deck ≥10 → play Poffin) vs O8 `drawfloor`, screened in parallel, ship ≤1; P0.2 kill-check on the bench premise | **O7: bench-0-after-t3 losses 14→1 of 12→6, Poffin-at-bench≤1 → ~90%, mirror 0.7113 n=800 z=+1.57 (point-above the live gac), dragapult 0.3375, kyogre 0.965, QC 3W–0L**; **O8 drawfloor KILLED in screens** (supporter under-play is weights-borne — 6th attempt); P0.2 killed a post-mortem misattribution (Dunsparce #305 has NO bench ability; the declined "ability" was the Nighttime Mine stadium) | 🚀 shipped 54935640 | live came back a **regression — implied ELO 714 vs M30's 817** (the offline battery mispredicted live again), which is what redirected M32 |
| [M32](M32.md) | 07-25 | deck surgery — redirected into a 3rd PPO transfer check | census of `opp_decks.parquet` (9,419 rows / 208 archetypes) → M32-B PPO warm-started from m28_winners against the decks we LOSE to (rocket/grim/mirror pool, dragapult sealed, archaludon sealed gate) | **NO-SHIP: PPO overfit rocket +5.3pp while breaching lucario −8.8pp z=−2.69 and archaludon −1.82 → strictly worse than m28_winners** = the **3rd reproduction of the no-transfer law**; the deck is **meta-optimal** — the top Alakazam variant `9294d9d8` (1,584 games, max 1251) is **byte-identical to our `clone54618168`**, so the entire 791→1251 gap is PILOT quality | 🔬 no-ship (strategic inflection) | BC ceiling ~0.66, O-rules spent, PPO-vs-exogenous dead a third time → the campaign turns to deck-tech and rules; M31 live resolved ~710 vs M30's 811 |
| [M33](m33-post-mortem.md) | 07-26 | value-guided closing (no-go) → setup-speed deck-tech | value-fn arc closed no-go (S1 premise falsified — value already 0.92 late; S3 self-play DAgger breaks replay-BC = the M24 wall) → `alakazam_v2`: +1 Rare Candy, +1 Night Stretcher, −1 Enhanced Hammer, −1 Nighttime Mine | **deck-tech V2 won: rocket 0.560→0.640 (+8pp z=2.3), grim 0.662→0.739 (+7.7pp z=2.4)**, non-inferior elsewhere; V1 KILLED; QC 2W–1L; live n=42 **implied ELO 743** vs M31's 714 — deck-outs fell 34.6%→12% of losses but the mass moved to opening bricks (35%) | 🚀 shipped 54997669 | the deck-tech did exactly what it was aimed at and the loss mass MOVED — a modest REAL gain over M31, still ~74 ELO below M30 |
| [M34](M34-plan.md) | 07-26 | consistency deck-tech to cut the opening-brick blowouts | Tier-0 combinatorial brick sim (`scripts/brick_sim.py`) + Tier-1 in-game brick signature via `offline_behavior.py`; 3 legal search-add variants vs alakazam_v2 | **NO-SHIP, all three fail**: agg (−2 disrupt) KILLED (rocket z=−3.9, the V1 lesson); ace (Master Ball) regresses grim z=−2.2; mine (−Nighttime Mine +Ultra Ball) holds every bed but is **NEUTRAL — the Tier-0 opening-brick −2.1pp does NOT convert in-game** (bench≤1 13.4→15.3%, worse, 2 seeds) | 🔬 no-ship | the pre-registered Tier-0/Tier-1 disagreement branch fired: **the brick is a PILOT problem, not a deck problem** (basic-play-at-bench≤1 only 39%) → redirected to M35's bench-floor rule |
| [M35](m35-post-mortem.md) | 07-26→27 | bench-floor pilot rule + the rules matrix | O10 `benchfloor` (bench a held basic at bench ≤1 — a deterministic card-fact, the allowed override class), ranked above poffinfloor; 4-arm matrix {gac, gacb, gacf, gacbf} n=400/bed | **gacf = the pick, holds ALL beds + a resolved behavior win (basic-play-at-bench≤1 36%→98%, bench-0-t3 loss share 27%→15%)**; gacbf KILLED (rocket z=−2.4, the composition law); gac also holds, so poffinfloor is droppable; QC 3W–0L; live n=55 **implied ELO 817 — best sub of the campaign (tied with M30), +74 vs M33 on a clean A/B** | 🚀 shipped 55011605 | benchfloor **TRANSFERRED offline→live almost exactly** (2 declined prompts in 55 games); the 2 residual bricks are draw variance (zero basics at every prompt) — **the brick lever is now genuinely spent, deck and pilot both** |
| [M36](m36-post-mortem.md) | 07-27 | anti-wall bed + O11 gustveto + hammer-4 deck-tech | extracted/verified the wall 60 (`decks/greattusk_wall.csv`), harvested crustle+tusk 609 seat-rows; battery {gacf, gacfv} × 7 beds; deck-tech h4 = +1 Enhanced Hammer / −1 Hilda | **h4 wall +6.0pp z=+2.46 resolved** (4-seed n=800, fresh-seed confirm, per-seed 0.430/0.425/0.415/0.415) + non-inferior on the other 6 beds; gacfv cleared every bed but was parked; QC 3W–0L → shipped; **post-mortem n=52: implied ELO 609, 0.500 vs a 608-avg field — BUSTED PLACEMENT, not a regression** (a new sub seeds ~600, drew 4 stalls in its first 6 games, then could not escape a band that is 40% stall/grim) | 🚀 shipped 55030954 | **a new sub does not inherit its predecessor's band** — the "clean live A/B" framing was structurally wrong, and sub-to-sub reads are band-confounded; within-family the h4 wall claim TRANSFERRED (0-4→2-3, live 0.40 ≈ offline 0.42); prize-array semantics inversion first spotted here and deferred |
| [M37](m37-post-mortem-tldr.md) | 07-28 | beat the 600 band: anti-stall race mode + a band-weighted gate | O12 racemode ladder — blanket racemoder KILLED (wall +14.8pp but hop −11.9 / garchomp −17.5), racemode2 KILLED (garchomp 5-seed z=−2.13) → **racemode3 = wall-family blanket only**; first-ever garchomp bed built from live subs | **racemode3 wall +15.7pp z=+5.50, probe-verified inert everywhere else (0 fires / 723 prompts)**, pooled +4.3pp; QC 9W–3L; live n=63 **implied ELO 736, 33W–30L — the 600 band WAS escaped** (8-6 in-band, 774 peak) but it settled at ~740, going 18-19 in the 700s and 1-3 above 800; the wall family still went 3-6, every loss a deck-out | 🚀 shipped 55065484 | mechanism law: blanket conserve wins vs NO-PRESSURE walls, starves setup vs attacking stall lines; **the +15.7pp was later invalidated — M39 showed it was measured on the `solver:wall` strawman bed and never existed on a BC bed, after three live milestones had paid for it** |
| [M37-audit](m37-code-audit.md) | 07-31 | forensic code audit of the M37 ship: post-mortem claims vs the actual code | every claim re-measured against the live `cg` engine rather than the `tests/builders.py` fakes | **the M36 prize-array inversion is SYSTEMIC — 4 sites, only 1 was known**: `rl/turn_solver.py::score_leaf` scored every knockout it could take at **−150,000**, poisoning the BC label corpus every shipped policy learns from (verified 2652/2652 prize leaves negative under the vendored old leaf, 1775/1775 positive after) | 🔬 audit, no ship | **EPOCH MARKER — every solver-backed bed number before 2026-07-31 is pre-fix and NOT comparable to post-fix numbers**, including the 0.55 `solver:lucario` campaign bar; the M38 recommendation ordering was inverted by this finding |
| [M38](M38.md) | 07-31 | retrain on the corrected teacher → pivot to a high-band winners fine-tune | Phase 0 kill-gate on the corrected leaf (A0 vendored-inverted control vs A1–A3) → G1 semantic prize-or-win bar → G3 teacher retrain (4k games, 337,698 labels) → pivot: 1000 fresh Kaggle episodes, 231 winner seats @800–1058 | kill gate PASSED (kill-plan commit ×3.9, 0.070→0.27); **G3 teacher-retrain FAILED decisively — ft/scratch/surgical all 0.123–0.264 vs control 0.517–0.631: the champion descends from KAGGLE WINNERS, not the solver corpus**; the pivot `m38_w9294_cont3` (champion + ~1 epoch) scored **pooled +6.1pp z=+6.03** (dragapult +17.1, iono +13.9, mirror +6.1; rocket −6.0 = watch item); QC 11W–1L | 🚀 shipped 55146658 | the **dose law** measured both directions the same day (~1 epoch gains, 10 epochs regresses); G4 `m38_bc_wall` bed delivered (champion 0.332); **G5: the rules stack is actively HARMFUL vs wall on the new net** (plain .323 > gacf .278 > gacfr3 .254) → the strip decision handed to M39; live settled 664.8 |
| [M39](M39.md) | 08-01→02 | campaign target raised to implied ELO 1000; build the instrument + the lane that can exceed demonstration | P0 weighted 9-bed gate + per-rule strip ablation + `m39_bc_top` (329 seats @≥900) → Ship A = rules strip → P2 racemode2/4 pair → P3 five policy arms across two corpora (vs-loss A, manufactured best-response B) + retention mixes | **Ship A: conserve-only vs live +1.74pp z=+3.42 → live 780.4 vs M38's 664.8, +115.6, best since M30/M35 — from ONE TOKEN in a config string**; Ship B: racemode2+4 pair +2.37pp z=+5.03 weighted, mechanism-proven (takes the wall opponent's own deck-out rate 3%→31%); **THE CEILING TRIGGER FIRED — all five policy arms negative on the 900+ panel (−0.9 to −5.9pp)** even though `m39_retain_b` was +1.85pp z=+4.01 overall | 🚀 shipped ×2: 55172160 (Ship A) + 55182097 (Ship B) | the binding constraint is **catastrophic forgetting, not volume and not demonstrator band** (retention mixes swing +3.9/+2.7pp with no new loss code); α=0.25 beats α=0 by +2.28pp so discarded loser rows carry signal; **G-12: `matchrunner --workers 8` is not run-reproducible** (.471–.537 at one seed) and **G-13: bed strength is a training lottery** (3 clones of one list score .671/.623/.546) → panels, not single beds; docs/VALIDATION.md born; four of the milestone's own conclusions were corrected mid-flight |
| [M40](M40.md) | 08-02→03 | leave the band: search, encoder v4 revival, and the instrument audit | S6 planzero across 4 serve sites · S5 v4/OppMemory re-encode · S3 compression/retention panel cross · S2 self-play corpus · floor ship of `m39_retain_b`; then the M40b forensics pivot | **THE CENTRAL RESULT: bed ABSOLUTES are void as band claims — X5 measured clone retention at 0.20, CI [0.07, 0.33]** against the plan's asserted 0.33–0.50, so a "1000+" bed is an ~800-band opponent and **the campaign has never had a working high-band instrument** (including M39's 0.532 that redirected slot 3); the floor ship came back a **live regression, 17W–30L = 0.36 / 659.7** and retain_b was retired; **S5 KILLED** (fidelity probe 100.0000% over 3,902 prompts but +0.04pp z=0.08 vs a matched control); S6 null → pre-registered adopt; composites +125 ELO [+90,+163]; vveto killed −4.6pp z=−11.5 | 🚀 shipped 55185485 (floor, regressed), then M40b r1 pair 55216935 + 55216941 | gate DELTAS survive (same fixed draw both arms), absolutes do not; **no offline instrument reproduces a live loss family** (positive control 4-for-4, composites inert vs us); `m40_top1000` was the first positive mirror-ceiling move (+2.5pp z=+2.12); G-14 ship_verify check 6 landed and failed on the live shipped agent before going green |
| [M41](M41.md) | 08-03 | separate DECK strength from PILOT strength, and build the deck-engineering surface | triggered by the leaderboard census (top 250 is **49.5% grim**, we are **2-13** against it; archaludon = 13.6% of bed weight and appears ZERO times up there) → Deck Lab (`tcg/cardpool.py`, `tcg/decklab.py`, `app/deck_lab.py`, 125 tests) + fixed-pilot `scripts/deck_probe.py` + `scripts/watch_games.py` replay bridge | **SHIPPED the campaign's first deck switch** — `m41_ogerpon` on `decks/ogerpon.csv` (47 seats / 3.3k decisions), on a thin bed delta and Piotr's "ship it as a floor, it doesn't cost us anything"; the QC battery earned its keep by catching a bundle exported from a mid-edit tree (first run 0-3/0-3, clean re-export 17W–10L); three card-data traps caught on first contact (`is_ex`/`is_mega_ex` disjoint, evolution is by NAME not id, Cinderace starts from hand) | 🚀 shipped 55221491 | explicitly **a floor and an option, NOT a validated improvement** (the +20pp is a bed delta and X5 voids bed absolutes; read at n≥150 per G-10); the sample agents are **not** deck-agnostic (hard-coded card-id cascades), and matchrunner cannot produce replays |
| [M41b](M41b.md) | 08-04→05 | train the encoder blocks nothing had ever trained (width 143), and test distilling the search | Stage A: a census found no checkpoint on the box had ever trained above width 100, so the M41 (100–104), M42 (105–114) and M41b (115–142) blocks were dead columns → `m41b_wide_prod` (14 epochs, seed 3, `--init-wide` from cont3) against a matched-dose w100 control, both specs pre-registered before the nets existed; Stage B: distil-the-search at two teacher budgets | **Gate 1 width in isolation +2.78pp z=4.02 (all 3 beds positive, replicated 3/3 seeds); Gate 2 vs the live champion cont3 +6.17pp z=8.77** (of which ~+3.4pp is the retrain itself — 23,115 rows vs M38's 16,082 — not the encoder); **the width gain VANISHES on a corpus cut to ogerpon's size (−0.34pp) → it is DATA HUNGER, not lineage specificity** (~10k decisions is not enough, ~23k is); every ogerpon retrain regressed (−5.35pp at 14 ep, −11.48pp at 2 ep) so the proven net shipped unchanged; **Stage B distillation grim-negative or inconclusive → BACKLOG #10 struck DEAD** | 🚀 shipped ×2: 55265099 (alakazam) + 55265105 (ogerpon) | the M38 "1 epoch gains / 10 epochs overwrites" dose law is **substrate-bound and did not transfer** (both arms improved 6→14 epochs); +6.17pp is an OFFLINE bed delta and X5 voids bed absolutes — read live at n≥45 per G-10; `m41b_wide_prod` is an ALAKAZAM net and must not be shipped on another deck |
| [M42](M42.md) | 08-03/04 | five named perception defects: measure, then fix | Piotr named five defects from the M41 ogerpon QC replays; each got a G-11 ladder in `scripts/m42_perception_probe.py`, run on the rule pilot and then on the actual neural ship | **NO-SHIP capability milestone — FOUR OF FIVE ARE NOT DEFECTS IN THE RULE PILOT**, and establishing that took SIX instrument corrections (retreat_stranded read 45% then 183%, is really 0 of 189; wasted_stadium is rule-IMPOSSIBLE; stadium_ignored is 58–87% per PROMPT but 0–62% per TURN; dead_basis_fetch 40–46% → 0 of 29); only hand_scaler_blind survived, at 100%. **THE DECISIVE CELL: the alakazam neural ship reads over_attach 9.8% / dead_basis_fetch 12.1% / stadium_ignored 50% where a rule pilot on the identical deck and opponent reads 0.0% / 0.0% / 10% — 440 damage forgone**; O19 `deadenergy` mechanism-proved 9.8%→4.2%, shipped OFF by default | 🔬 NO-SHIP capability milestone | **the heuristics already encode every rule; the net inherits none** — M41's finding from the opposite direction, with the teacher now measured at zero; delivered `rl/scaling.py` derived from engine attack text (6→63 entries), a 10-slot option block 105→115, `scripts/m42_column_safety.py` (23,409 real options, columns [0,100) bit-identical); housekeeping: `test_bundle_twins` compared only plan.py, which is why the M41 twin divergence survived |
| [M43](M43-plan.md) | 08-05→ | from imitation to game-play learning: self-play corpus + honest PPO | Piotr's directive: BC clones are exhausted (M39 ceiling, M40 X5 retention 0.20, M41b Stage B distillation dead) — pilots now train through GAMES. **Executed 08-12 on the NEW box** (amendment: artifact chain rebuilt from recipes, `m41_ogerpon` recovered exactly from the tracked bundle, specs re-registered `*_r1`). **Phase 0**: wide ship 55265099 read **823 implied ELO** [729,911] — NOT a 4th inversion, width-143 live-validated; ogerpon 650 with wall/arch live holes; benched-out mode live-confirmed vs arch. E0 chain: wide head 0.455 KILL → rebuilt retain_b 0.723 PASS → head transplant onto wide body 0.559 FAIL → **A-phi dropped by rule 0.c** (no valid Φ), critic warm-start kept. **Lane B: CLOSED INCONCLUSIVE** (+0.36pp then +0.04pp with doubled 328k-row corpus; the recovered control ceilings 15/19 beds — battery can't resolve; carry-forward: headroom beds needed). **Lane A A-base**: 50×400 honest PPO, all inner kills passed, peak vs_teacher **76.0%** (+6pp over frozen start, 3 promotions), supporter-credit hole reads closed | — | ✅ SHIPPED 55464234 (A gate PASS +3.72pp z=5.93; QC clean; Piotr go 08-12) | offline deltas are SCREENING only; ship criteria include Piotr's replay review, NOT waived; box lessons: OMP pinning = 15× matchrunner throughput, pool wedge fires at workers 8 (stall-detector + bounce procedure in M43.md), sync gaps recovered via git-tracked parquets |
| [M44](M44-plan.md) | 08-13 | competitive PPO league across (pilot, deck) pairs — BT-ELO selection, top-2 ship | Piotr r4 directive: 4 pairs (K alakazam / L mega-lucario NEW / O ogerpon / G grim), ELO from pilot-vs-pilot round-robin (tuned pinned 1000), TWO ships TODAY; pausable PPO (3a-3c) + loss-cause telemetry (3g, engine RESULT reasons, 5 buckets) + m44_league.py (3h) landed first, 1316 tests green. L floor-KILLED (0.020 vs iono — no lucario-capable pilot exists, escalated). Legs (15-it compression): K promoted it9 .790, G it9 .455 (did NOT transfer, −26 league), O it14 .505. Final n=800 table: K 1119.6 / O 1076.5 / tuned 1000* / iono 991.5 / G 893.0 / L 529.5 → K+O selected. Gates: O PASS +5.51pp z=9.31; K KILL −2.18pp z=−3.55 vs own seed (QC corroborated: 0-3 grim sweep). 3 external kills survived via --resume (3/3) | — | ✅ SHIPPED 55485260 (O ogerpon, gate PASS, QC 21W-6L clean) + 55485268 (K alakazam, Piotr explicit ship-anyway call on the kill — live A/B vs seed 55464234 answers transfer) | in-leg vs_teacher promotion is a SCREEN — it can anti-correlate with the broad field (K +3pp teacher, −2.2pp 25-bed); league ELO ≠ bed-field read; O loses by BENCH-OUT ~20% (deck lever for next); prize-race dominates live losses; ship rituals scripted (m44_ship_ritual.sh) |
| [M45](M45-plan.md) | 08-13/14 | mega-lucario(+solrock) seat: 1100+ teacher harvest, clone lineage, ship attempt | Scouting found the ladder holds 1100+ lucario teachers (602 seats one list); G-13 clone floors PASS (d3: 0.585 tuned / 0.372 iono); probe tokens `model-pz-bf`/`-gv` added | Track A: THREE KILLS — wide retrain (−2..−3.6pp pooled), both fix probes negative (benchfloor −2.9pp / gustveto −3.2pp, inside the ±4.9pp bar, on the since-killed d3 lineage), ship gate −24.69pp z=−36 vs the alakazam incumbent | ❌ NO SHIP — lucario line closed (Q1: alakazam only) | the d3 clone lives on as a GATE BED (m45_bc_lucario_d3); kill results carried into M46's band panel as the lucario family cell |
| [M46](M46-plan.md) | 08-14→ | band-faithful offline opponents + pivotal-mistake guardrails (the 800-ceiling answer) | Post-mortem (m44-live-postmortem.md): 780+ band = grim .39/mirror .23 + wall/stall/dragapult at wr≈0, pooled wr 0.30; two new gate→live inversions. Track A: 1100+ winners hand-aware corpora → G-13 clone beds (grim/mirror/drag/stall/wall) + band-weighted panel that DECIDES (n=83 weights hashed in spec, null-calibrated family-kill bar, A0v acceptance vs the live 810-vs-735 truth); Track B/C: deck-agnostic guards (dudguard0/attackfloor/retreatguard/bosscombo/benchzero + C3 mined rule facts). Piotr: best-sub scoring; guards-only bank ships EARLY 08-15 (`model-cz-ashw-dg0` on the 810 config) | A0v acceptance FAILED — instrument INVALID (weighted +0.24pp z=+0.58 vs live m41b+75elo, MDE 1.2pp; stall family-kill z=−9.79 the only live-consistent family; panel → ADVISORY, B2-B4 locked). B5 string restore KILL (−0.96pp z=−2.29, bootstrap all-negative, agreement 1.000 — Piotr's revisit call). Bank floors PASS (dg0 −1.3/+5.5pp vs ctl), t0 screen clean; ci_gate/ship_verify PASS; QC 18W-9L zero crashes (grim 0-3 sweep investigated: n=3 luck) | ✅ SHIPPED 55516471 (bank: m41b_wide_prod + conserve,planzero,ash,ashguard,dudguard0 + alakazam_v2_h4 — Piotr replay-review go 08-15, guards-only fallback per A0v rule) | the pre-registered acceptance test CAUGHT its own instrument's invalidity before any probe was believed (A0v design worked); weighted pools + anchors both fail as live predictors (3rd inversion class); string restores don't transfer across nets (B5); zsh mangled `--arm=...:c` spec args — single-quote matchrunner strings, verify emitted JSON |
