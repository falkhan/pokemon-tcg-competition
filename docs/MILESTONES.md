...
| [M40](M40-plan.md) | 08-02 | **STUB — not scheduled.** Framed by M39's fired ceiling trigger (imitation exhausted at 900+, all 5 arms negative on the panel) and by the slot arithmetic: 2 slots left, slot 4 is reserve, so M40 gets ONE real shot at a ~200 ELO gap. Candidate phases: **E0 value net on the harvest corpus** (the enabler debt three parked sweep items share — promoted to headline), E1 advantage-filtered BC (blocked on E0; retention x alpha=0.25 is the free untested cross), E2 self-play best-response (only if E0 passes; M39's version failed because 8% uniform exploration produced a corpus the net already agreed with 93.5% of the time), **E3 opponent-deck inference** (cut from M39 with its design intact, and now more valuable because the rules lane is the campaign's best-measured lever and E3 is the only way to reach the kanga/stall blind spots that have no buildable bed). Retention and alpha=0.25 stop being arms and become DEFAULTS. **Decision 1 is the milestone: slot 3 = bank the already-gated `retain_b x package` (+3.47pp, z=+7.44, G-4 paid) or bet it on the E0->E2 lane.** Recommendation: bank it, run the lane offline. Decision 2: does the 1000 target still stand at a ~200 ELO gap with one shot. Decision 3: VERIFY the slot budget — the '4 remaining' figure has never been re-checked. Nothing finalises before Ship B's n=150 read, whose wall cell carries a falsifiable mechanism prediction (opponent deck-outs ~30% live). | — | 📝 stub, awaiting Piotr |
