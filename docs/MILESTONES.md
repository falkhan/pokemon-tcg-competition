...
| [M43](M43-plan.md) | 08-05→ | from imitation to game-play learning: self-play corpus + honest PPO | Piotr's directive: BC clones are exhausted (M39 ceiling, M40 X5 retention 0.20, M41b Stage B distillation dead) — pilots now train through GAMES. **Lane B first** (ogerpon self-play corpus via `m40_s2_collect`, retention-mixed α=0.25; gate `docs/specs/m43_laneB.json` hash 9da112f64aaa77f8), then **Lane A headline** — the campaign's FIRST honest PPO measurement (the M2–M4 kill was the multiplicative-loss bug; arms A-base + A-phi, Φ=V(s) from the FROZEN start via `--shaping value`; gates c21d3351e44de0e9 / ad5930925ddf024b, control m41b_wide_prod, 25-bed roster). Phase 0 forensics first: live reads of 55265099/55265105 at n≥45 + E0 on the wide value head | — | 📋 planned, in flight | offline deltas are SCREENING only (three consecutive live inversions); ship criteria include Piotr's replay review, NOT waived; non-goals pre-registered: no AZ/PSRO, no distillation, no event-bonus rewards, no served search, no deck switch; pipeline-review prerequisites (`--phi-ckpt`, inert-shaping guard, tag-scoped `past=`) landed 08-11 — see `docs/m43-ppo-pipeline-review.md` |
