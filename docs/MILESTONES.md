...
| [M42](M42-plan.md) | 08-03 | five perception defects the pilot cannot see: measure, then fix | IN PROGRESS. Triggered by Piotrs QC replay review of the M41 Ogerpon ship. Phase 0 review finds only ONE of the five is a plain missing feature: hand-size scaling is absent everywhere (`has_variable_attack` is one undifferentiated bit over 181 cards; my hand size exists only as state idx 1625 in v4, with no option x state term). Two are worse than missing -- the rule pilots ONLY over-attach cap (`score_attach` tier 3) is UNREACHABLE on any printed-0 attacker because it gates on `_turns_to_ready == 0`, the same `if dmg <= 0` M41 traced through seven features; and past its 3-cost the same penalty PUNISHES Ogerpon for correct play, the exact false positive Piotr killed in the forensics on 08-03 which was never carried into the pilot. The basis-less-fetch guard exists (`fetch_value`, FETCH_DEAD_EVOLUTION=60) but leaks three ways (KEEP contexts only, soft not a ban, counts the actives name so "0 bench AND 0 basics in hand" triggers nowhere) and the neural ship has NO coverage at all since `apply_play_overrides` returns early off MAIN. Stadium presence is NOT the gap -- it is already encoded (state 126-161 identity pool, stadiumPlayed 1621, M27 option col 95); the gaps are stadium EFFECT in the damage model and stadium reasoning in the rule pilot. Also recorded, not fixed: `rl/encoders.py:207/:717` resolve inPlayArea targets against `your_index` instead of `opt.playerIndex`. Scope: probes + rule-pilot fixes + neural capability, NO collect/train/ship.
