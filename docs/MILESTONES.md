...
| [M36](M36-plan.md) | 07-27 | anti-wall bed + O11 gustveto + hammer-4 deck-tech | IN PROGRESS. P0: wall 60 extracted+verified (`decks/greattusk_wall.csv`); **strawman FULL PASS** (m35 config 0.33-0.35 vs generic/solver wall; diag 28/28 losses = deck-outs — live farm mode reproduced); harvest fix → crustle+tusk 609 seat-rows (clone viable); walls field-wide = anti-Alakazam farm (0.62 vs archetype, 0.25 vs grim); telepath alarm dismissed (per-TURN 87.2% ≈ M31); mirror race-conserve designed+PARKED. **E4: Powerful Hand = 20×hand counters, NO weakness, ZEROED by wall special energy → Enhanced Hammer is the mechanical unlock**; prize-array semantics pinned (.prize = own needs; _make_plan comment inverted — latent feature bug, queued for a training arm). P1: O11 gustveto implemented + tests; battery {gacf,gacfv}×7 beds n=400: **gacfv CLEARS all beds** (widest rocket −4.3pp z−1.24; engagement ~1/15 mirror games, all-in-losses). P2: **h4 (+1 hammer/−1 Hilda) wall +6.0pp z=+2.46 resolved** (4-seed n=800, fresh-seed confirm per pre-registered amendment; per-seed 0.430/0.425/0.415/0.415) + non-inferior on the other 6 beds; mechanism diag: new early bench-out win line. E1a portfolio (77% coverage): gacf 0.655 / gacfv 0.648 / h4 0.669. Piotr picked **h4** → QC 3W-0L vs sample-agent-tuned → **SHIPPED sub 55030954** (deck md5 02d22dca verified from tarball; gacf rules unchanged; clean live A/B vs 55011605 @789.6 isolates the hammer-4 wall tech). gacfv parked measured+ship-ready. **[Post-mortem](m36-post-mortem.md) (07-28, n=52): implied ELO 609 (0.500 vs 608-avg field) — BUSTED PLACEMENT, not a regression: new sub seeded ~600, first 6 games drew 4 stalls (all lost, deck-outs) → trapped in the stall/grim-saturated 600 band (40% of opponents vs ~24% in M35's 759 band). Within-family: h4 crustle claim TRANSFERRED (0-4→2-3, live 0.40 ≈ offline 0.42); mirror 8-4; 4 wins by winning the deck-out race (new line); benchfloor/deckguard/−1 Hilda all validated; no shipped change regressed. Loss mass: deck-outs 9/26 (broader stall family 0-5 where hammer is blind), grim 5, bricks 4 (pure draw variance). Leaderboard selection stays 55011605/54929991.** | 55030954 | 🚀 shipped |
| [M37](M37-plan.md) | 07-28 | beat the 600 band: anti-stall race mode + band-weighted gate | IN PROGRESS (phases run same-day on the Windows box; Piotr's kickoff calls: racemode arm, grim in trigger, laptop sync, grim-tech deferred, AWR greenlit). P0: O12 racemode/racemoder + pre-registered m37_decide bars + AWR `--outcome-weight` hook (B7 smoke PASS 45.3% non-win rows); B4 amended pre-decode (workers-8 not run-reproducible; fire probe = authoritative instrument, 0 fires/1313 mirror prompts). P1: garchomp BC clone built from live subs 54964845/55042761 (159 seats, val_acc 0.626; screen 0.470; strawman diag reproduces BOTH live modes incl. the 1-prize deck-out agonizer) — first-ever garchomp bed, accepted as directional. P3 battery (3 arms x {hop,wall,garchomp} x 3 seeds x n=200): **blanket racemoder wall +14.8pp z+5.20 but hop -11.9 z-4.19 / garchomp -17.5 z-6.17 KILLED; margin racemode safe everywhere but +1.25pp < bar**. P3b racemode2 (blanket-walls/margin-pressure, fresh controls): wall **+15.7pp z+5.50**, hop -0.5, garchomp 5-seed confirm **z-2.13 KILL** -> pooled +2.0pp FAIL. **P3c racemode3 = wall-family blanket ONLY: +15.7pp z+5.50 on wall, provably+probe-verified inert vs all non-wall decks (0 fires/723 prompts), pooled +4.3pp, band composite +2.1pp, zero regression risk. Mechanism law learned: blanket conserve wins vs NO-PRESSURE walls, starves setup vs attacking stall lines.** Sync landed (grim/rocket beds reproduce pins 0.650/0.610; racemode3 provably inert vs both). Export gacfr3 + from-tarball verify (deck md5 ad014c58 h4, hammer x4/Hilda x3, racemode3 vendored; bundle bit-identical weights to M36 — npz reconstruction exact). Multi-deck QC 9W-3L (3-0 tuned/iono/dragapult; prevship 0-3 investigated = mirror variance: provably-identical agents, pooled self-play 10W-17L/27, 12% tail). Piotr replay review + explicit go -> **SHIPPED sub 55065484** (dist/submission_neural_20260728_223734.tar.gz; single variable vs 55030954 = racemode3 rule). model_monitor.ipynb updated (M36+M37 rows + h4 deck meta; new standing rule: update on every submit). Watch: wall-family live W/L (offline 0.41->0.57, the transfer claim), band escape vs 600 seeding, hop/garchomp unchanged (rule inert), grim cell (M38 deck-tech target), gustveto still parked. **[Post-mortem](m37-post-mortem.md) (07-29, n=63): implied ELO 736 (33W-30L, avg opp 715.8) — band escape ACHIEVED (600-699 went 8-6), new equilibrium ~740 (700-799: 18-19; post-peak tail 7W-14L). racemode3 mechanically PERFECT live (9/9 wall games trigger-true, 0 demoted-draw uses/~410 offers) but the wall transfer claim FAILED: wall 3-6, ALL deck-outs (one at a 4-0 prize lead), races lost by 3-21 cards. Burn audit: our 2.5-2.7 cards/own-turn vs opp 1.4-1.5 — the leak is the engine (evolve triggers, Dawn 2.4, Rare Candy 3.0, Enriching Energy 4.0/attach, Poké Pad), not dud/fez. solver:wall bed = strawman-INVALID for economy fixes (solver over-digs itself); walls live at 689-836, not a 600-band artifact. NEW: mirror 3-5 with 3 deck-race deck-outs (one lost by a single card) — the parked M36 margin raceconserve is the candidate, measurable on the model-vs-model mirror bed today. Grim self-resolved 6-2 (zero code change; deck-tech deprioritized); garchomp 1-1, hop absent (inertness confirmed); gustveto residual 2 plays. Loss mass: deck-out 10/30, swept 9 (dragapult 1-3, hand-starved), contested 9, bricks 2. M38 ranked: wall BC clone (P0, blocking) → racemode4 (wider demote: Enriching/Poké Pad/Ash timing) → universal margin raceconserve for mirror → AWR headline (60% of loss mass is policy-quality) → band-gate re-freeze on the 700-800 mix.** | 55065484 | 🚀 shipped |
| [M37-audit](m37-code-audit.md) | 07-31 | forensic code audit of the M37 ship: post-mortem claims vs actual code | **The M36 prize-array inversion is SYSTEMIC — 4 sites, only 1 known.** Semantics re-verified live twice (winner's OWN array reaches 0; 58/58 prize drains belong to the acting seat) — M36 right, `sample-agent/main.py:283` WRONG (and `_root_snapshot` cited it as authority, which is how it entered). **P0 `turn_solver.score_leaf`: W_PRIZE/W_MY_PRIZE terms SWAPPED** — over 41,570 real leaves the opp-prize term fired **0** times (unreachable dead code) while **1291/1291** of our own KOs scored **NEGATIVE** (1-prize KO = -150,000, 3-prize = -450,000). `_dfs` seeds best_score with stand-pat (~0) and only replaces on `>`, so **the solver could never choose a prize line** unless it won outright; MIN_OVERRIDE_SCORE unreachable -> the M7.4a multi-prize-lethal headline was INERT in production. Blast radius = the BC corpus (`plan_iter:109` solve_turn_line generates the expert labels) — the teacher was labelling *don't take prizes unless it ends the game*, which is exactly M37's unexplained 30% swept + 30% contested-race loss mass and the agent's dig/deck-out shape. Missed for 30 milestones because `test_turn_solver.py` encoded the SAME inversion on `tests/builders.py` fakes, which can drop op_prizes on our own turn — a transition the real engine never produces. **Green suite was not evidence.** FIXED; post-fix 0/832 prize leaves negative, solve_turn override 0/18 -> 7/18. **P0b NEW (surfaced by the fix): `MIN_OVERRIDE_SCORE = W_PRIZE-1` is MIS-CALIBRATED** — W_COUNTER/W_RACE/W_DECK_LOW ride the same total, so any negative tiebreak >1pt drops a real 1-prize line under the bar (6 of the 7 new overrides are outright wins, only 1 is a prize line). NOT retuned — tuned constant, needs its own pre-registered A/B (DEV_OVERRIDE_MARGIN precedent); pinned by test so retuning fails loudly. **Top M38 follow-up: P0 is only half-landed without it.** **P1 `collector:456` + `selfplay:229`: PPO prize shaping sign FLIPPED** (delta == the encoder's own 'negative = I'm ahead' expression) — rewarded the OPPONENT taking prizes. Affects PPO arms (M22c, M28 C1), not the BC ship. FIXED both twins. **P2 `plan.py` wins/concedes (M36's known one): MEASURED and it's minor** — over 1,288 real plan candidates `wins` disagreed **0.0%**, `concedes` **0.2%** (lethal/return_ko gates absorb it). Left unfixed deliberately (frozen-net feature shift); M36's defer call was right, but it should NOT outrank P0/P0b as the M38 plan currently has it. Drift fixed: `tcg/__init__` claimed rl.export/rl.gate were 'the live path' (inverted — build_submission drives tcg.shipping); **`rl.export` CLI was a live footgun** (never synced `submission/rl/` -> ships stale plan/encoders SILENTLY; M18.1/M22c fossil-deck class) — both CLIs now fail loudly, importable API kept for the parity tests; CLAUDE.md paths + submission_rules docstring corrected. NOT dead despite 0 inbound imports: `rl/behavior.py` and `rl/gate.py` are CLI instruments (grep-count deletion would have been a mistake). Clean: ship state byte-identical to `rl/*`, deck md5 matches h4, all 6 gacfr3 fix names resolve, card-fact sets non-empty (benchfloor CAN fire), racemode3 matches its documented behaviour. Suite 662 pass / 1 pre-existing env fail (no Kaggle creds). **M38 sequencing consequence: recalibrate the bar -> regenerate solver corpora with fresh controls -> THEN the AWR arm** (running AWR first fits it to the same poisoned labels). No re-ship: nothing here changes shipped inference. | — | 🔍 audit |
| [M38](M38.md) | 07-31 | corrected-teacher retrain → pivot: high-band winners fine-tune | SHIPPED **55146658** (`m38_w9294_cont3` + gacfr3 + alakazam_v2_h4; dist/submission_neural_20260731_211047.tar.gz; single variable vs 55065484 = the net). **Phase 0 battery (A0 vendored-inverted leaf vs A1-A3): kill gate PASSED** — E0c kill-plan commit ×3.9 (0.070→0.27), E0a A1/A3 pooled z≈+2.3/+2.4, E0b no over-greed (no W_COUNTER retune). **G1 landed: A3 semantic prize-or-win gate is the default bar** (Piotr's call; `_cleared` shares it). G2 fresh pins: champion .839 vs corrected solver:lucario (0.55-era bar dead), plain m28_winners .844 (gacfr3 adds nothing there), grim .706, rocket .595. **G3 teacher-retrain FAILED decisively**: gen-1 clean corpus (4k games, 337,698 labels, value_solve off) → ft/scratch/surgical ALL collapsed or killed (.123-.264 vs control .517-.631) — the champion descends from KAGGLE WINNERS, not the solver corpus; imitating the corrected teacher hurts at any dose (localized-prior claim right about location — init val 0.451 on prize rows — wrong about direction). **Pivot (Piotr): kaggle winners harvest** — 1000 fresh episodes, hash 9294d9d8 231 winner seats @800-1058 → `m38_w9294_cont3` = champion + ~1 epoch: **pooled +6.1pp z=+6.03 over control** (iono +13.9 z+5.8, dragapult +17.1 z+7.2, m28 +7.9, mirror +6.1, grim +4.3; rocket −6.0 z−2.4 = live watch item). Dose law both directions same day (10 ep regresses, 1 ep gains). **G4 wall clone bed DELIVERED** (`m38_bc_wall`, 138 seats, val 0.676): champion 0.332 vs wall — loss mode confirmed; winners corpus does NOT fix it; **G5: the rules stack is actively harmful vs wall on the new net** (plain .323 > gacf .278 > gacfr3 .254) with no measurable gain elsewhere — stack kept per minimal-diff, strip decision → M39. P2 plan.py wins/concedes fix shipped (twin re-exported). QC 11W-1L (3-0/3-0/3-0 + 2-1 prevship); probe PASS 474/474; Piotr replay review + explicit go. Watch: rocket cell, wall unchanged, band trajectory vs 736. | 55146658 | 🚀 shipped |
| [M39](M39-plan.md) | 08-01 | **campaign target raised to implied ELO 1000** (top pilot of our own list 9294d9d8 sits at 1058); attempt aimed at slot 3, not the next ship | build the instrument + the lane that can exceed demonstration, and recover the M38 regression. **4 submission slots left**: 1 = Ship A (strip, alone), 2 = Ship B (net x rules), 3 = the 1000 attempt (M40), 4 = untouched reserve. P0: harvest r3, m39_bc_grim/stall beds (retires the M26 relic), weighted gate on `data/m39_live_mix.json` (G-3), **P0.7 six-cell per-rule strip ablation**, **P0.8 THE INSTRUMENT** = `m39_bc_top` cloned from 903-1058 seats (runs concurrent with Ship A prep, gates P3 arm training only; pre-registered readings <=10 / 10-25 / >25pp decide whether slot 3 is an imitation or a self-play attempt). P1 Ship A = **full strip to G5-plain, ablation-verified** (**correction: gacf IS deckguard/ash/conserve/benchfloor** — 'keep the economy rules' was an UNMEASURED config, not a conservative one). P2 anti-deck-out (racemode2 trigger coverage + racemode4 burn demote); P2c opponent-deck inference CUT to M40. P3 two corpora in parallel — A harvested vs-loss (built WITHOUT `--winners-only`: the drafted AWR arm was a verified **no-op**, alpha=0 reproduces winners-only exactly) and **B manufactured best-response vs our own clone beds = the headline lane**, since BC cannot exceed its demonstrator band; + `m39_retain` (data-mixing vs the 1-epoch dose law). **[VALIDATION.md](VALIDATION.md) NEW standing protocol**: computed CIs on our own ship records for the first time — **M37 [646,819] and M38 [566,753] overlap across nearly their full width**, and the headlined wall cell (3-6 -> 1-5) is Fisher p=0.60; at n=50 a live read detects only a ~200 ELO swing. Four-tier pre-ship battery (artifact correctness **from the tarball**, mechanism proof, weighted strength gate, QC-as-smoke-test) + pre-registered post-ship read schedule. New guardrails G-7 (one lane per slot) G-8 (band coverage) G-9 (statistical honesty; no matchup-level gating from live data at any n) G-10 (dwell **declared per ship before submit** — Piotr's call) G-11 (mechanism proof per ship). All 13 decision points settled 08-01. **P0 COMPLETE 08-01** (diary docs/M39.md): live mix n=147 exposed the PLANNED roster at **77.5% coverage — below the G-3 floor**; **archaludon measured at 12.2% of live games with no bed** -> `m39_bc_archaludon` built (137 seats, val .744), coverage 82.3%. **`m39_bc_stall` KILLED as unbuildable** (9 seats at band, 22 all-bands, still 9 after the snowball; and ZERO 800+ winner seats against it). `m39_bc_grim` 195 seats val .662 retires the M26 relic (G-6). Harvest r3 +600 episodes -> seats 2594->3908; **vs-loss falsification test PASSES** (grim 76 / wall 27 winner seats @800+ on our list => the loss mass is a piloting gap, stay-on-alakazam confirmed on data). **INSTRUMENT `m39_bc_top`** (329 seats @>=900, val .682): champion **-8.7pp**, cont3 **-2.7pp** vs the 900+ band = the BEST pre-registered reading (<=10pp). Extended on Piotr's call: `m39_bc_topgrim` (grim @>=1000, val .674, same list as the 700-850 grim bed = a controlled pilot-skill ladder); wall high-band NOT buildable (57/35 seats). **Weighted gate reproduced M38's failure mode on a new net**: cont3 vs champion unweighted +3.2pp but **weighted +0.14pp** (carried by dragapult +19.9pp at 4.8% share and iono +11.4pp at 0.7%, while lucario -7.3pp at 19.7%). **SHIP A PREMISE UNSUPPORTED**: strip vs live = **-1.10pp +/-1.73, z=-1.24, no detectable difference** (helps wall +2.5pp pooled z~+2.2, leans negative on lucario/mirror) -> Piotr called INVESTIGATE FURTHER; 3-arm n=2400 battery running. **Strip ledger: FULL STRIP** per the pre-registered rule, no rule cleared the keep-bar — but the `conserve` MECHANISM probe then found it fires 13x/12 games on the 900+ bed, 8x mirror, **0x wall, 0x grim**, matching exactly where its removal cost WR (-2.75pp top vs -0.29pp wall); an inert rule cannot cost 2.75pp, so that line is likely to be overturned. **NEW STANDING RESULT G-12**: `matchrunner --workers 8` is NOT run-reproducible (5 identical runs at seed 1: .471/.494/.526/.506/.537), so the campaign's habitual n=800 2-seed cell resolves only ~10pp while rules effects are ~5pp — **two of my own P0 conclusions were corrected as a direct result**. **SHIP A SHIPPED 55172160** (`m38_w9294_cont3` + fix string **`conserve`** + alakazam_v2_h4; dist/submission_neural_20260801_234656.tar.gz; single variable vs 55146658 = the fix string). **Decision 1 (full strip) was OVERTURNED BY MEASUREMENT**: Piotr called investigate-further on the weak Ship A premise, and the powered 3-arm gate (9-bed weighted roster, n=2400/cell) gave **conserve-only vs live +1.74pp z=+3.42**, conserve-only vs full-strip +1.03pp z=+2.03, plain vs live +0.71pp n.s. — positive on 6/8 beds incl. every big-share one. **A MECHANISM PROBE, not a significance threshold, got this right**: scripts/conserve_probe.py showed conserve fires in 11-13% of trigger-true states on the 900+ and grim beds but 0.2% on wall — active exactly where its removal cost WR. G-11 earned its keep. Pre-ship: new scripts/ship_verify.py Tier-1 battery all PASS, tcg.shipping gate OK, prize probe 1144/1144, QC 16W-8L over 8 opponents incl. the new wall/grim/archaludon/top900 legs; Piotr replay review + go. **G-10 dwell DECLARED BEFORE the sample: decision read n=150** (~3 days, +/-56 ELO; expected effect ~+12 ELO so a 1-day +/-93 read could not resolve it). **D1 found G-13: bed strength is a TRAINING LOTTERY** — three clones of the same grim list from near-identical data score .671/.623/.546 vs the same net (12.5pp spread vs a 2.0pp binomial CI; one fewer epoch moves it 4.8pp). Gate DELTAS are insulated (same fixed bed both arms) but every ABSOLUTE bed number carries ~+/-6pp until beds become panels — free via redistributing match budget across >=3 draws. **G-13 unimplemented; it is the prerequisite for the P2/P3 gates.** Also refuted en route: 'BC cloning saturates' (head-to-head z=+12.04 — cloning transmits skill fine). rl/replay_bc.py gained --opp-deck-hash (+2 tests). **Four of my own conclusions were corrected mid-milestone, all the same shape (small-n read as settled) — recorded in the diary, not quietly fixed.** New docs/VALIDATION.md standing protocol + G-7..G-13. **[08-02] G-13 IMPLEMENTED** (scripts/m39_build_panels.sh + m39_panel_gate.sh + panel support in m39_decide.py): 7 new draws in 2.5 min, budget REDISTRIBUTED not added (n=1200 x 3 draws instead of n=2400 x 1). First run paid for it — the wall panel spreads **10.8pp** (.296/.188/.197) against a 2.2pp binomial CI and **`m38_bc_wall`, the bed EVERY wall claim in this campaign was measured on, is the SOFTEST draw by ~10pp**; archaludon's draws agree to 1.8pp, so construction variance is per-family, not a constant. The 900+ ceiling panel reads **0.532, i.e. +3.2pp ABOVE parity** (P0.8's single draw said −2.7pp) — the "band is reachable" branch strengthens. **P2 (rules lane): `racemode2` alone KILLED** (−1.3pp on the wall panel; and this finally reconciles M37's famous +15.7pp wall gain, which was measured on the `solver:wall` bed M37's own post-mortem later called strawman-INVALID — the gain never existed on a BC bed, and three live milestones paid for it). **`racemode4` (NEW, demotes OUR measured burn: Enriching Energy 4.0 cards/attach, Poke Pad, surplus Dawn/Hilda once Alakazam is out) +1.48pp z=+3.15. The PAIR is +2.37pp weighted, z=+5.03, wall panel +11.2pp (+15.7/+11.1/+6.7 across draws) — SYNERGISTIC, and neither ships alone**: racemode2 closes the draw-ABILITY exits, racemode4 the burn PLAY/ATTACH exits, and closing one alone just pushes the policy through the other (which is why racemode2 solo is negative). **G-11 mechanism (n=200/leg): the package takes the WALL OPPONENT's deck-out rate 3% -> 31% and their lowest deck 23.1 -> 7.9, vs <=5.5pp movement on the mirror bed where the rules provably never fire** — we do not burn less, we make the game last long enough that the wall's own burn kills it first (the M36 opp-deck-0 pattern, never before reproduced on a faithful bed). P2a id work: Mega Kangaskhan ex added to the wall/blanket half (the BACKLOG's kanga blindspot — all 10 kanga lists, 30 of 65 wall lists), Fan Rotom to the pressure/margin half (7 of 16 stall lists, but 2 of 83 MIRROR lists, hence margin-gated); TR Kangaskhan ex + plain Kangaskhan rejected on harvest evidence; new tests/test_race_bed_inertness.py pins which bed can fire which half. **P3 (policy lane): corpus A built (172 seats, 12k decisions, NOT winners-only per finding #9); corpus B needed a NEW collector — the plan's `plan_iter collect` recipe labels with the SOLVER TEACHER in both modes and would have re-run M38's known kill**, so scripts/m39_collect_bestresp.py records our own net's executed actions with 8% exploration and an outcome filter (2,400 games, 172k rows). **SHIP B EXPORTED 08-02, NOT SUBMITTED**: `m38_w9294_cont3` + `conserve,racemode2,racemode4`, deck alakazam_v2_h4 — a **ONE-LANE rules change** (G-7-clean, which matters because Ship A is at n=10). dist/submission_neural_20260802_021047.tar.gz; ship_verify all PASS incl. a NEW behavioural probe that runs the BUNDLE's own plan module in a subprocess (proves the rule fires, not just that its name parses); tcg.shipping gate OK; prize probe 2402/2402; QC **19W-8L** over 9 opponents incl. a new `wall_hard` leg (the hardest wall draw) which went 3-0. Ship B dwell declared n=150 before the sample. **Ship A live at n=10: 5W-5L, implied ELO 741, CI [557, 926] — no verdict permitted (G-9).** **P3 COMPLETE (5 arms, panel roster): both corpora LOSE alone (A −3.10pp z−6.51, B −0.85pp) and BOTH are rescued by mixing champion shards back in — retain_a +0.80pp, `m39_retain_b` +1.85pp z=+4.01, swings of +3.9 and +2.7pp from data mixing alone with no new loss code. So the binding constraint was CATASTROPHIC FORGETTING, not volume and not demonstrator band — a third answer the pre-registered A/B diagnostic did not enumerate, and it re-reads M38's "~1 epoch dose law" as a symptom of coverage loss rather than the constraint itself. The α-ladder was pre-registered as "expect a null" and is NOT: α=0.25 beats α=0 by +2.28pp, so the ~50% loser rows the harvest has discarded since M28 carry substantial signal (qualifies, does not overturn, the winners-only law — and both retention arms ran at the WORSE α=0). **THE CEILING TRIGGER FIRES: no arm moved the 900+ panel — all five negative (−0.9 to −5.9pp)** — so per the plan's own branch **slot 3 becomes a self-play / enabler-debt (value net → real advantages) milestone, not another corpus iteration.** Ship B stays the RULES candidate (+2.37pp z=+5.03, mechanism-proven, net untouched) over the net candidate (+1.85pp, no fire probe possible); `m39_retain_b` is slot 3's strongest input and its × package cross is gating.  **SHIP B SUBMITTED 08-02: sub 55182097** (`m38_w9294_cont3` + `conserve,racemode2,racemode4` + alakazam_v2_h4; dist/submission_neural_20260802_021047.tar.gz; single variable vs 55172160 = the fix string; one lane so G-7 needed no dwell; dwell declared n=150 before the sample). **AND THE LADDER ANSWERED SHIP A: 55172160 = 780.4 vs M38's 664.8, +115.6 — best since M30/M35's 817, from ONE TOKEN in a config string.** What produced it was the mechanism probe, not the plan: decision 1 had settled FULL STRIP and the P0.7 ledger discarded `conserve` at z=-1.91 as one-of-12 contrasts; conserve_probe.py predicted which beds it could act on BEFORE the deciding battery ran, the powered gate found +1.74pp z=+3.42, and the ladder paid +115. G-9 still applies to good news: one number, no interval, declared dwell n=150 not reached. **Pre-registered success bar (decision 12): "780 with a clear slot-3 thesis is a successful M39; 820 with no attribution is not" — 780.4 with the thesis written and evidenced.** | 55172160 + 55182097 | 🚀 both M39 ships live |
| [M40](M40.md) | 08-02 |  **DAY 1 EXECUTED (08-02).** SHIPPED **55185485** (`m39_retain_b` + conserve,racemode2,racemode4 + alakazam_v2_h4; single variable vs Ship B = the net; live 782.4 @ n=25; read-n 45). **THE MILESTONE'S CENTRAL RESULT: bed ABSOLUTES are void as band claims.** X5's 3x3 panel cross measured clone retention at **0.20, CI [0.07,0.33]** against the plan's asserted 0.33-0.50 -- so compression is WORSE than assumed, D1's 0.623 keystone falls OUTSIDE the panel CI, and draw variance is 2.8x sampling variance (G-13 applies to compression too). Consequences: a '1000+' bed is an ~800-band opponent, the 900+ MIRROR bed is deck SYMMETRY not band, and **the campaign has never had a working high-band instrument** -- every 'we play the N-band to X' claim in the record, including M39's 0.532 that redirected slot 3, measures one of those artifacts. Gate DELTAS survive (same fixed draw both arms). **S6 DECIDED**: `planzero` across FOUR serve sites (plan said three), mechanism-proven (0 plan-head runs vs a control feeding a non-zero plan on 35% of prompts -- the defect surface, never measured before), battery NULL on both lineages (+0.43 z=0.91; +0.80 z=1.72) firing the pre-registered null->adopt branch; the n=200 screen that motivated it did NOT reproduce. **G-14 landed** (ship_verify check 6, required --corpus) and FAILED on the live shipped agent while checks 1-5 passed -- then went green. **S5 KILLED**: walker + `build --v4` + fidelity probe PASS at 100.0000% over 3902 prompts, but vs a MATCHED v3 control the encoder is +0.04pp z=0.08 (0.664 vs 0.666 in-sample) -- bench-threat blindness was real and fixing it changed nothing; the '43% slow-setup' mechanism story is unsupported. **S3 CONSTRUCTIVE HALF: composites work** -- +125 ELO [+90,+163] over the clone they wrap, **2.6x the entire harvestable demonstrator band range**, and the only remaining route to a target-band opponent (per-spec solver budget shipped). **X0**: reads do NOT split (parallel is real) but submissions DIE -- lifetime min 36 / median 50 / max 112, verified against the LIVE API on four dead subs, so **n=150 has never been reached** and G-10's declared read-n was an unreachable bar; replication, not patience, is the route to a decision-grade n. **S2**: E0 PASSES (loss-family matched-pair 0.642, 68% of value variance WITHIN games) and is an EVALUATION job not a training one; resumable chunk-checkpointed collector with policy-temperature exploration (agreement 0.805 vs M39's 0.935); arm 1 gate flat (+0.09pp z=0.19) but the per-cell table shows pool beds UP and non-pool beds DOWN -- two real effects cancelling, i.e. opponent-distribution overfit, not nothing. The advantage-weighted branch E0 unblocked is still untried. **METHOD FINDING: four times in one day a NEW instrument gave a confident wrong answer before the code under test did** (S6 probe counted the opponent's plan head; the fidelity probe read 26.4% and would have killed the lane; G-14's v4 check asserted the wrong one-hot offset with a unit test encoding the same misreading; E0 fed state_ids=zeros) -- treat a decisive negative from a new instrument as evidence about the instrument. Also self-inflicted: S6 removed a train/serve mismatch in the morning and the S2 collector re-introduced it six hours later, because G-14 gates ship time and nothing gates collection time. 735 tests pass (+48 new). **DECIDED v3, IN PROGRESS.** Leave the ~150-point band; campaign target implied ELO >=1000 converged (n>=150 per G-10). Piotr's decision round settled all 8 open points: **S1 solver on the neural line REJECTED** (stop-invest stands; the solver stays an instrument for S3 composites and S2 opponent pools, never a pilot component); **S6 battery runs and on a NULL we serve plan=0 anyway** as pure risk-removal, bundled with the next net ship; **the floor (`m39_retain_b` x package, +3.47pp z=+7.44, G-4 already paid) ships now** as single-lane-vs-Ship-B with a parallel n=150 read; **S2 runs as a FULL lane in-milestone**; target stays 1000 with S1's framing corrected to TWO hypotheses - H-A (BC clones compress their demonstrator's edge, so every bed we own is ~700-band wearing a higher label) vs H-B (we are at parity with MIRROR clones and simply lose to non-mirror families) - which the milestone's first job is to build an instrument to discriminate. Day-1 scope (Piotr): X0 accrual audit -> S6 end-to-end -> S5 walker + fidelity probe; floor ship prepped to QC and STOPPED for replay review; scripts fork to `m40_*` so M39's instruments stay frozen as the record of the M39 verdict. **Code verification before implementation produced 8 corrections to the plan** ([diary](M40.md)): S6 is FOUR serve sites not three (`rl/collector.py` carries a fifth copy of the plan state machine); the plan's claim that a v4 corpus **will not load** with v3 champion shards is FALSE - the M21 shim zero-pads it, so the hazard is a silent all-zero 341-dim v4 block that `encode_ctx_v4` can never emit at serve time (measured: 16,082 such rows in a mixed load) - the SAME defect shape as S6, which is why G-14 must cover both and not just the encoder version; the walker's skip surface measured at ~0.11% of own prompts (434/27,152 over 200 episodes, only ~29 mid-game with logs) so it is cheap insurance, not a crisis; the solver budget has been per-call since M11 so S3 is ~6 lines; the topgrim ceiling bed already exists on disk; and `runs/m39_retain_b.log` reads **init val_acc 0.907**, so S2's '>0.90 kills the corpus' gate MUST run at collection or it kills the milestone's best net. Also measured: **a full 72-cell panel-gate arm costs ~24 minutes** - offline batteries are not a constraint this milestone, code is. Standing gap flagged: `verify-before-consequential-actions.md`, which CLAUDE.md requires reading before any ship, does not exist anywhere. **Stub thesis retained for the record:** **STUB — not scheduled.** Rewritten same-day on Piotr's steer that a modest increment is not worth building for. **Thesis: we are not stuck on policy quality, we are stuck on the INSTRUMENT.** Two facts that cannot both be about our policy — we beat `m39_bc_top` (a clone of 900-1058-band pilots of our own list) at **0.532**, and we are **0-6 live vs 800+ opponents** across M37+M38. M39's D1 head-to-head accidentally measured the compression: the 1000-band grim clone beats the 750-band clone 0.623 (z=+12.04), ~86 ELO of separation from demonstrators ~250 ELO apart, so **a BC clone retains roughly a third to a half of its demonstrator's edge**. Every bed we own is therefore ~700-band wearing a higher label, and five milestones have optimized WR against the band we are already in. Candidates that do NOT inherit that ceiling: **S1 — give the neural line the within-turn combo solver it has never had** (the shipped bundle has been a greedy one-action argmax since M11; `turn_solver` was written to fix exactly that and only ever shipped in the RULES bundle. Screen 08-02, n=30: `solved` 0.533 vs `model-conserve` 0.300 on the wall bed — +23pp on our worst matchup, mean move 5.9ms of a 50ms budget. **SCREEN ONLY**, n=30 resolves ~35pp; and it reopens a documented stop-invest line, so it needs Piotr's sign-off); **S2 — self-play** (needs E0 value net + a real exploration design; M39's version failed because 8% uniform exploration gave a corpus the net agreed with 93.5% of the time); **S3 — build opponents ABOVE the clone ceiling** (clone+solver composites) and re-run the P0.8 gap read, which is the direct test of the thesis and the prerequisite for believing S1/S2 at all. **Slot structure that makes the swing rational: slot 3 = the step change, slot 4 = the already-gated `retain_b x package` (+3.47pp, z=+7.44, G-4 paid) as a floor that does not expire.** Decisions: sign off on S1; S2 now or M41; VERIFY the slot budget; target stays 1000 unless S3 refutes the thesis. **S5 ADDED 08-02 — we shipped opponent memory in M21 and silently regressed off it in M24.** Found while answering whether the pilot sees opposing card ids / hp / damage. v3 (our live net) gives every opposing slot its card id at an embedding site, 36 card features, current hp and maxHp, energy count and per-type energies, tools — plus best-affordable-damage/KO flags for their ACTIVE only. v4 adds `_slot_extras` (12 slots x 5, including **best affordable damage PER SLOT — the opponent's BENCH threat, which v3 never projects**) and `OppMemory` (last-4 opp played card ids, last attacker, last energy-attach target — 'their charging target is their announced next attacker'). The lineage lost it by accident, not by decision: from M24 every corpus comes from `replay_bc`, which encodes with encode_state_v3 and **never reads obs.logs at all**, so every net since is v3 (m38_w9294_cont3 has no enc_ver; input 1708 = 1361 numeric + 27 plan + 20 ids x 16). **The data is already cached — 828/844 (98.1%) of replay observations carry non-empty logs** — so this is a re-encode, not a collection campaign, and `migrate_v3_to_v4` + the M21 zero-init warm-start invariant means the vehicle is low-dose fine-tune (what works), not a fresh train (what has always collapsed). Risk that gates the lane: OppMemory's contract is locked to the LIVE prompt window, so **first deliverable is a fidelity probe** — reconstruct memory features offline for a game we also have live and assert they match, because an encoder that lies is worse than one that is blind. **S5 re-prices E3: do not scope the opponent-deck classifier until S5 is priced.** M40's shape is now two missing-faculty candidates (S1 search, S5 encoder — opponent-independent, so neither inherits the clone ceiling) plus the instrument work (S2/S3/S4). **M40b (08-03): forensics-driven pivot — floor read = live REGRESSION (0.36, retain_b retired); no offline instrument reproduces live losses (positive control 4-for-4, composites inert vs us); S2 arms 2a/2b killed/null; deep dive: fixed t8 setup clock, winners-only exposure holes, loss modes = speed (38/82) + grind (30/82). SHIPPED r1 pair: **55216935** (cont3 + conserve,planzero,ash+ashguard — racemode dropped on live evidence) and **55216941** (m40_top1000 1000+-mirror fine-tune, same rules; mirror ceiling +2.5pp z=+2.12, first positive ceiling move). vveto (test-time value) killed −4.6pp z=−11.5; grim-clone ship killed at sanity bar 0.436; read-n 45 each, replicate pooling. | 55216935 + 55216941 | in progress |
| [M41](M41-plan.md) | 08-03 | separate DECK strength from PILOT strength, and build the deck-engineering surface | Triggered by the leaderboard census: the top 250 is **49.5% grim** against 9.7% of our bed weight, we are **2-13 vs grim** live, and `archaludon` — 13.6% of bed weight, three dedicated beds — appears **zero** times up there. Typing is structural: the Marnie's Grimmsnarl line is weak to **Grass**, our Alakazam is weak to **Darkness**, and 42.0% of the non-grim top field is already Grass. `docs/BACKLOG.md` holds the three-way decision (switch to grim / engineer an anti-grim deck / keep teaching ours); the blocker is that **every measurement to date confounds deck with pilot** — M40b's Track A 0.436 kill measured our NET PILOTING a grim clone, while the same note records the DECK at 0.70 H2H. P1 Deck Lab (`tcg/cardpool.py` + `tcg/decklab.py` + `app/deck_lab.py`, Streamlit via `uv run --with`): card browsing/filtering, deck building with live legality, save to `decks/custom/`, subprocess smoke test. P2 fixed-pilot deck probe (`scripts/deck_probe.py`) — both sides on `generic:`, opponents weighted by TOP-250 share not our live mix, so the delta is the deck. P3 replay bridge (`scripts/watch_games.py`) closing a real gap: matchrunner emits no `visualize` payload, but `rl/eval.py::play_games` accepts the callables `make_pilot` returns, giving watchable replays AND postmortem-readable JSON. P4 loss analysis → rules. **Two planning corrections landed in BACKLOG.md**: the sample agents are NOT deck-agnostic (hard-coded card-id cascades — the probe would have measured the brain's card coverage, not the deck), and matchrunner cannot produce replays. | — | in progress |
| [M42](M42-plan.md) | 08-03/04 | five perception defects: measure, then fix | **NO-SHIP capability milestone.** Piotr named five defects from the M41 ogerpon QC replays. FOUR OF FIVE ARE NOT DEFECTS IN THE RULE PILOT, and establishing that took SIX instrument corrections -- retreat_stranded read 45% then 183% (rungs 2/3 were independent ifs) and is really 0 of 189; wasted_stadium was RULE-IMPOSSIBLE (same-name replacement is illegal, 0 same-id offers in 20); stadium_ignored read 58-87% per PROMPT and is 0-62% per TURN; dead_basis_fetch read 40-46% and is 0 of 29 once in-play promotions and multi-selects are excluded (a benched Kadabra needs no Abra -- a rule written against that would have refused to promote after a KO). Only hand_scaler_blind survived unchanged, at 100%. THE DECISIVE CELL: the same ladders on the ALAKAZAM NEURAL ship (model-c-pkg:m39_retain_b, Piotr's actual configuration, which nothing had probed) read over_attach 9.8% / dead_basis_fetch 12.1% / stadium_ignored 50% against a rule pilot at 0.0% / 0.0% / 10% on the IDENTICAL deck and opponent, 440 damage forgone. The heuristics already encode every rule; the net inherits none -- M41's finding from the opposite direction, with the teacher measured at zero. Delivered: scripts/m42_perception_probe.py (5 G-11 ladders); 3 post-mortem flags + rate-normalised batch; rl/scaling.py DERIVED from the engine's attack text (6 -> 63 entries, 15 modes, allow-list, 4 of 6 curated re-derived independently and the 2 it cannot are pinned human overrides); a 10-slot option block 105->115; `scaling` finally reaching score_attach (it never had fixes passed at all); and O19 `deadenergy` OFF by default, mechanism-proved at 9.8% -> 4.2% single-variable. scripts/m42_column_safety.py makes the append-and-slice proof a command: 23,409 real options, columns [0,100) bit-identical. Housekeeping: test_bundle_twins compared only plan.py (why the M41 divergence survived) and ship_verify.TWIN_FILES was missing the bundled scaling.py. 1062 tests pass, same 4 pre-existing failures.
| [M43](M43-plan.md) | 08-05 | from imitation to game-play learning: self-play corpus + honest PPO | Piotr's directive: BC clones are exhausted (M39 ceiling, M40 X5 retention 0.20, M41b Stage B distillation dead) — pilots now train through GAMES: vs other pilots and self-play. **Lane B first** (ogerpon self-play corpus: close the measured ~13k-row width-143 data gap with `m40_s2_collect` rows, retention-mixed, α=0.25; gate `docs/specs/m43_laneB.json` hash 9da112f64aaa77f8, control m41_ogerpon), **Lane A headline** (the campaign's FIRST honest PPO measurement — the M2–M4 kill was the multiplicative-loss bug; arms A-base + A-phi per BACKLOG #13(c)'s pre-registered falsification order, Φ=V(s) from the FROZEN start via new `--shaping value`; gates m43_laneA_base c21d3351e44de0e9 / m43_laneA_phi ad5930925ddf024b, control m41b_wide_prod, full 25-bed roster). Phase 0 forensics first: live reads of 55265099/55265105 at n≥45 + E0 on m41b_wide_prod's value head (`scripts/m43_forensics.sh`). Offline deltas are SCREENING only (three consecutive live inversions); ship criteria include Piotr's replay review NOT waived. Non-goals pre-registered: no #9 AZ/PSRO, no #10 distillation, no event-bonus rewards, no served search, no deck switch. | — | 📋 planned |
