"""M40: weighted-pool gate decoder — the G-3 instrument.
...
Fork of scripts/m39_decide.py (Piotr, 2026-08-02). The M39 decoder emitted
the verdict that shipped Ship A and Ship B and stays frozen as the record
of it; this fork is free to grow beds. The statistics are UNCHANGED --
same n-weighted family pooling, same pooled-SE two-proportion z vs the
same-battery control, same 0.80 coverage floor, same G-13 spread report.
The only addition is a SECOND ceiling panel (below).

Usage:
    uv run python scripts/m40_decide.py --arm cont3_pkgz --control cont3_pkg
    uv run python scripts/m40_decide.py --arm X --control Y --prefix m40_pan
"""
import argparse
import glob
import json
...
BED_FAMILY = {
    ...
    # scripts/m39_live_mix.py — no statistic changes, keys only; this
    # decoder stays frozen otherwise and the m46 instrument reads neither
    # copy, its family map lives in the hashed band spec).
...
# M40 S3 -- the NON-MIRROR high-band ceiling panel, and the H-B test.
# TOP_BEDS clones 900+ pilots of OUR OWN list, so parity there is parity
# with a mirror. The live 800+ losses were grim, dragapult, archaludon --
# non-mirror families. If we sit at parity against top mirror clones and
# well under against top GRIM clones, matchup structure (H-B) explains the
# live record with no compression (H-A) needed at all. Reported apart from
# BOTH the gate and the mirror ceiling: three numbers, never pooled.
TOPGRIM_BEDS = ("topgrim_d1", "topgrim_d2", "topgrim_d3")
...
def ceiling_panel(args, beds, title: str, note: str) -> None:
    """Report one ceiling panel apart from the gate (G-8).

    Extracted from the M39 decoder's inline 900+ block so M40 can run TWO of
    them with identical arithmetic. Neither is ever pooled into the weighted
    number: a ceiling bed is not in the live mix, so it is a selection signal,
    never a ship gate.
    """
    rows = [(b, cell(args.prefix, args.arm, b), cell(args.prefix, args.control, b))
            for b in beds]
    rows = [(b, a, c) for b, a, c in rows if a and c]
    if not rows:
        return
    if len(rows) < len(beds):
        # G-13: a partial panel is one lottery ticket wearing a family's name.
        print(f"\n--- {title}: PARTIAL ({len(rows)}/{len(beds)} draws) — "
              f"read as a single draw, not a panel ---")
    na = sum(n for _, (_, n), _ in rows)
    nc = sum(n for _, _, (_, n) in rows)
    pa = sum(p * n for _, (p, n), _ in rows) / na
    pc = sum(p * n for _, _, (p, n) in rows) / nc
    print(f"\n--- {title} (NOT pooled into the gate) ---")
    print(f"  {note}")
    for b, (p, n), (q, _m) in rows:
        print(f"  {b:<12} arm {p:.3f}+/-{ci95(p, n):.3f}   control {q:.3f}"
              f"   delta {p - q:+.3f}")
...
def main() -> int:
    ...
    ap.add_argument("--prefix", default="m40_pan")
    args = ap.parse_args()
    ...
    if not uw_n:
        # M40: the M39 decoder divided by zero here. Not a real defect there
        # (it only ever ran on finished batteries), but M40 decodes batteries
        # mid-flight to watch them, so an empty roster must report rather than
        # traceback — a crash reads as "broken", which is not the same as
        # "nothing has finished yet".
        print(f"\nNo completed cells for arm '{args.arm}' / control "
              f"'{args.control}' under prefix '{args.prefix}' yet.")
        return 1
    ...
    # ---- the ceiling instruments, reported apart from the gate (G-8) ------
    # Two panels, never pooled with each other or with the gate. The pair is
    # the H-A vs H-B discriminator: mirror-parity WITH a non-mirror gap is
    # matchup structure; a gap on both is compression.
    ceiling_panel(args, TOP_BEDS, "900+ MIRROR CEILING PANEL",
                  "our own list at 900+ -- parity here is parity with a MIRROR")
    ceiling_panel(args, TOPGRIM_BEDS, "1000+ NON-MIRROR CEILING PANEL (grim)",
                  "the H-B test: a NON-mirror family at the band that beats us "
                  "live")
