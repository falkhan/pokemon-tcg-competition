...
def detect_opp_deck(steps, seat):
    """The opponent's 60-card list from their first recorded action (the env
    contract: the first action each agent returns IS its deck), or None."""
    for step in steps[:4]:
        st = step[1 - seat] if 1 - seat < len(step) else None
        if isinstance(st, dict):
            act = st.get("action")
            if isinstance(act, list) and len(act) == 60:
                return [int(a) for a in act]
    return None


def adjudication_suspect(last_state) -> bool:
    """A loss that was never behind on the prize race at the final decision:
    prizes-left tied or better, own deck not empty, opponent still holding
    prizes — the suspected turn-cap adjudication shape."""
    me = last_state.players[last_state.yourIndex]
    op = last_state.players[1 - last_state.yourIndex]
    return (len(me.prize or []) <= len(op.prize or []) and me.deckCount > 0
            and len(op.prize or []) > 0)


def analyze_episode(decisions, is_wall_opp):
    """The per-decision measurement core over ONE episode.

    ``decisions`` yields (obs_dict, action) pairs — iter_replay_decisions
    output, pre-conversion. ``is_wall_opp`` gates the racemode3 accounting.
    Returns a dict: boss_plays [(turn, opp_prizes_left)], the wall trigger and
    dud/fez offer/use counts split by trigger state, the per-turn deck-race
    trajectory, and last_state (adjudication anatomy reads it after the loop).
    """
    boss_plays = []
    ...
    for obs_dict, action in decisions:
        ...
        if ct == OptionType.PLAY and chosen_cid == BOSS:
            boss_plays.append((st.turn, len(op.prize or [])))
        if is_wall_opp:
    ...
    return dict(boss_plays=boss_plays, traj=traj, last_state=last_state,
                trig_true=trig_true, trig_prompts=trig_prompts,
...
def main():
    ...
    for r in df.iter_rows(named=True):
        ...
        opp_deck = detect_opp_deck(steps, seat)
        is_wall_opp = bool(WALL_IDS & set(opp_deck or []))

        res = analyze_episode(
            ((od, a) for _i, od, a in iter_replay_decisions(steps, seat, drops)),
            is_wall_opp)
        boss_plays += [(ep, t, pz, reward) for t, pz in res["boss_plays"]]

        if is_wall_opp:
            wall_rows.append(dict(
                ep=ep, reward=reward, traj=res["traj"],
                trig_true=res["trig_true"], trig_prompts=res["trig_prompts"],
                offer_trig=res["offer_trig"], use_trig=res["use_trig"],
                offer_off=res["offer_off"], use_off=res["use_off"]))
        ...
        if reward == -1 and last_state is not None \
                and adjudication_suspect(last_state):
            me = last_state.players[last_state.yourIndex]
            op = last_state.players[1 - last_state.yourIndex]
