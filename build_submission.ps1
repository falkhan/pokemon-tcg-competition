...
#
# Build only:        .\build_submission.ps1
# Build + submit:    .\build_submission.ps1 -Message "M0 random-weights baseline"
...
Write-Host "[1/3] Export artifacts (rl.export)" -ForegroundColor Cyan
& $py -m rl.export
if ($LASTEXITCODE -ne 0) { throw "export failed" }

Write-Host "[2/3] Gates: parity, deck legality, Kaggle-style game (rl.gate)" -ForegroundColor Cyan
& $py -m rl.gate
...
$out = "dist\submission_$stamp.tar.gz"
tar -czf $out -C submission main.py deck.csv policy_weights.npz card_features.npy cg
...
foreach ($required in "main.py", "deck.csv", "policy_weights.npz", "card_features.npy",
                      "cg/api.py", "cg/utils.py", "cg/sim.py", "cg/libcg.so") {
    if ($contents -notcontains $required) { throw "package is missing $required" }
}
