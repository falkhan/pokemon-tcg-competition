...
# --- Tunable heuristic weights (M5: optimized via self-play; defaults = original agent) ---
# Overridden in-process by the optimizer; baked to chosen values for the submission. An
# optional sibling weights.json overrides them (so the packaged agent ships its tuned set).
W_DISCARD_SCALE = 60      # Mega Brave damage scaling per discarded Fighting Energy
W_PRIZE_PENALTY = 500     # penalty for attacking while at 2-3 prizes (don't feed Mega ex)
W_ACTIVE_ATTACKER = 220   # prefer attacking with the active (no switch cost)
W_ACTIVE_TARGET = 300     # prefer targeting the opponent's active
W_ENERGY_BASE = 8000      # baseline priority of attaching energy vs other actions
try:
    import json as _json
    _wp = os.path.join(os.path.dirname(os.path.abspath(__file__)), "weights.json") \
        if "__file__" in globals() else "weights.json"
    if os.path.exists(_wp):
        for _k, _v in _json.load(open(_wp)).items():
            globals()[_k] = _v
except Exception:
    pass
...
# Load the bundled deck.csv (robust: next to this file, or the Kaggle mount, or CWD).
_here = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else "."
for _dp in (os.path.join(_here, "deck.csv"), "/kaggle_simulations/agent/deck.csv", "deck.csv"):
    if os.path.exists(_dp):
        break
with open(_dp, "r") as file:
    csv = [ln for ln in file.read().split("\n") if ln.strip()]
                ...
                for a in range(2):
                    ...
                    if my_pokemon.id == Mega_Lucario_ex:
                        if a == 0:
                            energy_required = 1
                            base_damage = 130
                            base_score += W_DISCARD_SCALE * min(3, discard_counts[Basic_Fighting_Energy])
                        else:
                            energy_required = 2
                            base_damage = 270
                        if my_prize == 2 or my_prize == 3:
                            base_score -= W_PRIZE_PENALTY
                    elif a == 1:
                        break
                    ...
                    for j, op_pokemon in enumerate(op_cards):
                        ...
                        if i == 0:
                            score += W_ACTIVE_ATTACKER
                        if j == 0:
                            score += W_ACTIVE_TARGET
                        score += energy_count
                        if best_score < score:
                            best_score = score
                            plan.attacker = i
